...
class TestCommentRepository:
    ...
    def test_create(self, comment):
        with vcr.use_cassette(str(cassettes_path / "comment/create.yaml"), mode="once"):
    ...
    def test_update(self, comment):
        with vcr.use_cassette(str(cassettes_path / "comment/create.yaml"), mode="once"):
            response = self.client.create(self.project_id, comment)
        with vcr.use_cassette(str(cassettes_path / "comment/update.yaml"), mode="once"):
    ...
    def test_delete(self, comment):
        with vcr.use_cassette(str(cassettes_path / "comment/create.yaml"), mode="once"):
            response = self.client.create(self.project_id, comment)
        with vcr.use_cassette(str(cassettes_path / "comment/delete.yaml"), mode="once"):
