...
class TestDataImportRepository:
    @classmethod
    def setup_class(cls):
        with vcr.use_cassette(str(repository_fixtures / "data_import/login.yaml"), mode="once"):
    ...
    def test_list_options(self):
        with vcr.use_cassette(str(repository_fixtures / "data_import/options.yaml"), mode="once"):
            response = self.client.list_options(self.project_id)
        assert len(response) > 0
        assert all(isinstance(option, Option) for option in response)

    def test_upload(self):
        with vcr.use_cassette(str(repository_fixtures / "data_import/upload.yaml"), mode="once"):
            upload_id = self.client.upload(self.file_path)
        assert upload_id is not None
        assert isinstance(upload_id, str)

    def test_delete(self):
        with vcr.use_cassette(str(repository_fixtures / "data_import/delete.yaml"), mode="once"):
            upload_id = self.client.upload(self.file_path)
            self.client.delete(upload_id)

    def test_ingest(self):
        with vcr.use_cassette(str(repository_fixtures / "data_import/ingest.yaml"), mode="once"):
            upload_id = self.client.upload(self.file_path)
