...
class TestDataExportRepository:
    @classmethod
    def setup_class(cls):
        with vcr.use_cassette(str(repository_fixtures / "data_export/login.yaml"), mode="once"):
    ...
    def test_list_options(self):
        with vcr.use_cassette(str(repository_fixtures / "data_export/options.yaml"), mode="once"):
    ...
    def test_schedule_download(self):
        with vcr.use_cassette(str(repository_fixtures / "data_export/schedule_download.yaml"), mode="once"):
    ...
    def test_download(self):
        with vcr.use_cassette(str(repository_fixtures / "data_export/download.yaml"), mode="once"):
            option = Option(name="JSONL")
            task_id = self.client.schedule_download(self.project_id, option)
